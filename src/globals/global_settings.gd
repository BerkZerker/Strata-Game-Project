...
const MAX_CHUNK_REMOVALS_PER_FRAME: int = 16 # Max number of chunks to remove per frame
const MAX_BUILD_QUEUE_SIZE: int = 64 # Max chunks waiting to be built (backpressure threshold)
const MAX_CHUNK_POOL_SIZE: int = 128 # Max number of chunks to keep in the pool for reuse